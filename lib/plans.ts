...
// El tipo `Plan` y el catálogo de prestaciones viven en
// `plans.catalog.ts` para que el panel de admin (cliente) los importe
// sin arrastrar `./db` → `@libsql/client` al navegador. Se reexportan
// para no romper a los consumidores que ya importaban de aquí.
export { PLAN_FEATURES } from './plans.catalog'
export type { Plan, PlanFeatureKey } from './plans.catalog'
import type { Plan } from './plans.catalog'
