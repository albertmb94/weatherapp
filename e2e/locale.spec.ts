...
test.describe('esquema de URLs (navegador en español)', () => {
  ...
  test('el español no lleva prefijo y las URLs existentes siguen valiendo', async ({ page }) => {
    await page.goto('/')
  ...
  test('el inglés se sirve bajo /en con el lang correcto DESDE EL SERVIDOR', async ({ page }) => {
  ...
  test('/es/... redirige a la forma canónica sin prefijo', async ({ page }) => {
    const res = await page.request.get('/es/premium', { maxRedirects: 0 })
    expect(res.status()).toBe(308)
    expect(res.headers()['location']).toContain('/premium')
  })

  test('un idioma inexistente da 404, no una página en español', async ({ page }) => {
...
test.describe('negociación de idioma', () => {
  ...
  test('un navegador en inglés sin elección previa acaba en /en', async ({ page }) => {
    await page.goto('/')
    await expect(page).toHaveURL(/\/en$/)
    await expect(page.locator('html')).toHaveAttribute('lang', 'en')
  })

  test('un bot NUNCA se redirige: ve el sitio en el idioma por defecto', async ({ page }) => {
...
test.describe('metadata para buscadores', () => {
  ...
  test('cada página declara su propio canonical, no el del layout', async ({ page }) => {
  ...
  test('los hreflang enlazan la misma página en los dos idiomas', async ({ page }) => {
  ...
  test('el título y la descripción están traducidos', async ({ page }) => {
    await page.goto('/cookies')
...
test.describe('navegación entre idiomas', () => {
  ...
  test('el selector lleva a la ruta del otro idioma y conserva el estado de la URL', async ({ page }) => {
  ...
  test('volver a español quita el prefijo', async ({ page }) => {
    await page.goto('/en')
...
test.describe('enlaces internos', () => {
  ...
  test('conservan el idioma', async ({ page }) => {
...
test.describe('rutas exentas', () => {
  ...
  test('el panel de administración no lleva idioma en la URL', async ({ page }) => {
    const res = await page.request.get('/admin', { maxRedirects: 0 })
    // Redirige al login por falta de sesión, NO a /es/admin.
    expect(res.headers()['location']).toContain('/admin/login')
  })

  test('robots y sitemap siguen respondiendo', async ({ page }) => {
